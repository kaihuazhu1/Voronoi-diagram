...
// Calculates and prints watchtower information in sorted order by diameter
void *stage4(char *dataset, char *polygon, char *output) {
    tower_t **towers;
    ...
    *faces_size = 1;
                ...
                // after first three watchtowers implement traversal algorithm
                    ...
                    while(loop_temp != start) {
                        if (poly->faces[old_face-1].half_edge->twin->prev->twin != NULL && poly->faces[old_face-1].half_edge->twin->next->twin != NULL) {
                            ...
                            poly->faces[old].half_edge->next->twin = poly->faces[(*faces_size)-1].half_edge->prev;
    ...
    // insertion sort algorithm to sort towers by diameter
    for(int i=0; i<*lines; i++) {
        tower_t *v = towers[i];
        int j = i-1;
        while(j>=0 && towers[j]->diameter > v->diameter) {
            towers[j+1] = towers[j];
            j--;
        }
        towers[j+1] = v;
    ...
    for(int i=0; i<*faces_size; i++) {
        halfedge_t *loop_half = poly->faces[i].half_edge;
        for(int j=0; j<poly->faces[i].size; j++) {
            loop_half = loop_half->next;
        }
    }
